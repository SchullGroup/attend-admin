...
- `GET .../post-agm/statutory-return` — **Shipped, integrated.** Real field shape confirmed (`eventName`, `eventDate`, `eventVenue`, `organisationName`, `totalRegistered`, `totalAttended`, `totalVotesCastShares`, `resolutions[]` with `order`/`title`/`specialResolution`/`forVotes`/`againstVotes`/`abstainVotes`/`forShares`/`againstShares`/`abstainShares`/`passed`). Frontend now renders this as a formatted plain-text statutory filing document ("Export Data" button downloads a readable `.txt`, not a raw JSON dump).
