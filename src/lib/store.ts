...
export const useStore = create<AttendAdminStore>((set) => ({
  ...
  enrollRegistrar: (id) => set((s) => ({ registrars: s.registrars.map((stk) => stk.id === id ? { ...stk, status: "active" as const } : stk) })),
  suspendRegistrar: (id) => set((s) => ({ registrars: s.registrars.map((stk) => stk.id === id ? { ...stk, status: "suspended" as const } : stk) })),
