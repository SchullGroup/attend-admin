...
export default function RegistersPage() {
  ...
  const filtered = activeTab === "all" ? organisers : organisers.filter((s) => s.status === activeTab);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">Companies whose meetings and events are managed on the platform</p>
        </div>
        <Link href="/registers/enrol">
      ...
      <Card className="attend-card overflow-hidden">
            ...
            {filtered.map((stk) => {
              ...
              return (
                <tr key={stk.id} className="attend-table-row">
                  ...
                  <td className="px-5 py-3">
                    <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold" style={{ color: plan.color, backgroundColor: plan.bg }}>
                      {plan.label}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-sm font-medium tabular-nums text-[hsl(var(--foreground))]">{stk.eventsCount}</td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1.5">
                      <Link href={`/registers/${stk.id}`}>
                        <Button size="sm" variant="outline" className="h-7 text-xs">View</Button>
                      </Link>
                      {stk.status === "active" && (
                        confirmId === stk.id ? (
                          <Button size="sm" variant="ghost" className="h-7 text-xs text-red-600 bg-red-50 font-semibold" onClick={() => { suspendOrganiser(stk.id); setConfirmId(null); }}>Confirm?</Button>
                        ) : (
                          <Button size="sm" variant="ghost" className="h-7 text-xs text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => setConfirmId(stk.id)}>Suspend</Button>
                        )
                      )}
                      {stk.status === "suspended" && (
                        <Button size="sm" variant="ghost" className="h-7 text-xs text-green-700 hover:text-green-800 hover:bg-green-50" onClick={() => enrollOrganiser(stk.id)}>Activate</Button>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-sm font-medium text-[hsl(var(--foreground))] mb-1">No registers match this filter</p>
            {activeTab !== "all" && (
              <button onClick={() => setActiveTab("all")} className="text-xs text-[hsl(var(--primary))] hover:underline mt-1">Clear filter</button>
            )}
          </div>
        )}
      </Card>
