...
const CRITERIA = [
  {
    label: "Innovation",
    points: 25,
    desc: "Originality and novelty of the idea",
  },
  {
    label: "Feasibility",
    points: 25,
    desc: "Technical and business viability",
  },
  {
    label: "Technical Execution",
    points: 25,
    desc: "Quality of implementation and demo",
  },
  {
    label: "Business Potential",
    points: 25,
    desc: "Market opportunity and revenue model",
  },
];
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
            MeriHack 2026 — Score shortlisted teams
          </p>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[hsl(var(--primary)/0.06)] border border-[hsl(var(--primary)/0.15)]">
          ...
          <span className="text-sm font-semibold text-[hsl(var(--primary))]">
            {shortlisted.length} Teams to Score
          </span>
        </div>
      </div>
          ...
          {shortlisted.length === 0 && (
            <Card className="attend-card p-12 text-center col-span-2">
              ...
              <p className="text-sm mt-1 text-[hsl(var(--muted-foreground))]">
                Go to Applications to shortlist teams.
              </p>
            </Card>
          )}
            ...
            <Card key={app.id} className="attend-card p-5">
              <div className="flex items-start justify-between mb-4">
                ...
                {app.score != null && (
                  <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[hsl(var(--primary)/0.06)] border border-[hsl(var(--primary)/0.15)]">
                    ...
                    <span className="text-lg font-bold tabular-nums text-[hsl(var(--primary))]">
                      {app.score}
                    </span>
              ...
              {submitted[app.id] ? (
                <div className="flex items-center gap-2 text-green-600 text-sm font-medium">
                  ...
                  Score submitted:{" "}
                  <span className="font-bold">{scores[app.id]}/100</span>
                </div>
              ) : (
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <label className="attend-section-title mb-2 block">
                      Score (0–100)
                    </label>
                    <Input
                      type="number"
              ...
              {CRITERIA.map((c) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs font-bold text-[hsl(var(--primary))] bg-[hsl(var(--primary)/0.08)] rounded-full px-2 py-0.5">
                      {c.points} pts
                    </span>
                  </div>
