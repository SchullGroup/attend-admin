      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
            {data?.totalElements ?? events.length} total events across all registers
          </p>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="attend-table-header">
              {/* empty header for the type-circle column */}
          ...
          <tbody>
            {filtered.map((event) => {
              ...
              // Register = stakeholderName or registerName. Never organizerName (that's the registrar firm).
              const registerName = getEventRegisterName(event);

              return (
                <tr key={event.id} className="attend-table-row">

                  {/* ── Event-type colour circle ── */}
                  ...
                  {/* ── Title + module badge + organiser name ── */}
                  <td className="px-5 py-3">
                    ...
                    {/* Organiser = the register (organisation) that owns this event */}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4 px-1">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="outline" className="h-7 text-xs" disabled={page >= totalPages - 1}
              onClick={() => setPage((p) => p + 1)}>
              Next
            </Button>
          </div>
